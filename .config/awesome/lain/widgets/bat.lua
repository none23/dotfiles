...
local function worker(args)
  ...
  -- find the power sources names {{{
  local ac = "ACAD"
  local battery = "BAT1"

  local ps = io.popen("ls /sys/class/power_supply/")
  ...
  bat_notification_low_preset = { title = "The end is near"
  ...
  bat_notification_critical_preset = { title = "Hey, the battery is kinda low"
                                     , text = "You know how crappy the battery is..."
                                     , fg = "#000"
                                     , bg = "#de1e1e"
                                     , timeout= 5
                                     , position   = "top_right"
  ...
  function bat.update()
    ...
    local bat_present = first_line(bstr .. "/present")

    if tonumber(bat_present) == 1 then
      ...
      bat_now.perc   = energy_percentage or bat_now.n_perc
    ...
    if bat_now.status ~= "N/A" then
      -- update {perc,time,watt} if battery not full and rate > 0
